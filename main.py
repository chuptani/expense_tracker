...
#                 input_string.pop(0)
#                 self.ctx.change_date(input_string)
...
#             else:
#                 date = date_string[1]
#             if date in self.ctx.dayweeks.keys():
#                 day = self.ctx.dayweeks[date]
#                 days_since = (self.ctx.today.weekday() - day) % 7
#                 if days_since == 0:
#                     days_since = 7
#                 new_date = self.ctx.today - datetime.timedelta(days=days_since)
#             else:
...
#             if date in self.ctx.dayweeks.keys():
#                 day = self.ctx.dayweeks[date]
#                 days_till = (day - self.ctx.today.weekday()) % 7
#                 if days_till == 0:
#                     days_till = 7
#                 new_date = self.ctx.today + datetime.timedelta(days=days_till)
#             else:
#                 error("invalid date:", f"'{date}' is not a valid weekday")
#         elif is_prefix(date, "yesterday"):
#             new_date = self.ctx.today - datetime.timedelta(days=1)
...
#                     f"{self.ctx.today.year}-{date}", "%Y-%m-%d"
#                 ).date()
#             elif is_date(date, ["%d"]):
#                 new_date = datetime.datetime.strptime(
#                     f"{self.ctx.today.year}-{self.ctx.today.month}-{date}", "%Y-%m-%d"
#                 ).date()
#         else:
#             error("unknown date")
#             print(f"Current date: {self.ctx.current_date} | {weekday}")
#             return
#
#         self.ctx.current_date = new_date
#         self.ctx.weekday_num = self.ctx.current_date.weekday()
#         weekday = self.ctx.weekdays[self.ctx.weekday_num]
#         green(f"Date changed : {new_date} | {weekday}")
#
#     def valid_entry(self.ctx, input_string):
#
